...
local function dispatch_message(body)
    ...
    if not ok then
        skynet.error(string.format("[protocol] reject undecodable packet: %s", tostring(t)))
        return
    end
    if t == "REQUEST" and message_callback then
        ...
        elseif not status then
            skynet.error(string.format("[protocol] message handler error for %s: %s", tostring(name), tostring(result)))
        end
    end
end

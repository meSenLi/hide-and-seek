...
local function dispatch_message(body)
    ...
    if not ok then
        skynet.error(string.format("[agent_components.protocol] reject undecodable packet: %s", tostring(t)))
        return
    end
    if t == "REQUEST" then
        if message_callback then
            local status, result = pcall(message_callback, name, args)
            if response and status then
                send_frame(response(result))
            elseif not status then
                skynet.error(string.format("[agent_components.protocol] message handler error: %s", tostring(result)))
            end
        end
    end
    -- ignore RESPONSE
